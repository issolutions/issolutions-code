<?php 
/**
 * JigoShop Customising templates using hook
 *
 * @package WallPress
 * @since WallPress 1.0.6
 */
...
function wallpress_after_shop_loop_item_2() { ?>
	<a title="<?php the_title(); ?>" class="button button-white" href="<?php echo get_permalink(); ?>"><?php _e( 'More Details', 'wallpress' ); ?></a></div>
<?php }
...
<div id="jigo_minicart">
	...
	<div class="cartlist">
        ...
        <?php if ( ! empty( $cart_contents ) ) : ?>
			...
			<?php if ( $_product->exists() && $value['quantity'] > 0 ) : ?>
                <li class="clearfix">
                    ...
                    <div class="js_widget_product_price">
                        ...
                        <?php _e(' &middot; QTY','wallpress')?><?php echo $value['quantity']; ?>
                    </div>
                </li>
			<?php endif; ?>
			<?php endforeach; ?>
			...
			<p class="total">
				<strong><?php _e( ( ( get_option( 'jigoshop_prices_include_tax') == 'yes' ) ? 'Total' : 'Subtotal' ), 'wallpress' ); ?></strong>
			...
			<p class="buttons clearfix">
				<a href="<?php echo esc_attr( jigoshop_cart::get_cart_url() ); ?>" class="button">
					<?php _e( 'View Cart &rarr;', 'wallpress' ); ?>
				</a>
				<a href="<?php echo esc_attr( jigoshop_cart::get_checkout_url() ); ?>" class="button checkout">
					<?php _e( 'Checkout &rarr;', 'wallpress' ); ?>
				</a>
			</p>
		<?php else : ?>
			<span class="empty"><?php _e( 'No products in the cart.', 'wallpress' ) ?></span>
		<?php endif; ?>
		</div>
	</div>
</div>
